...
func main() {
	...
	for i := 0; i < fn2; i++ {
		...
		fr[popcount(i)] = append(fr[popcount(i)], t)
	}
	for i := 0; i <= sep; i++ {
		sorti(fr[i])
	}
	if n <= sep {
		li := bs(0, len(fr[k])-1, func(c int) bool {
			return fr[k][c] < l
		})
		ri := bs(0, len(fr[k])-1, func(c int) bool {
			return fr[k][c] <= r
		})
		out(ri - li)
		return
	...
	for i := 0; i < ft2; i++ {
		...
		li := bs(0, len(fr[k-pc])-1, func(c int) bool {
			return fr[k-pc][c] < l-t
		})
		ri := bs(0, len(fr[k-pc])-1, func(c int) bool {
			return fr[k-pc][c] <= r-t
		})
		o += ri - li
	}
